    ...
    <aside id="mini-cart-panel"
        ...
        <div id="mini-cart-footer" class="border-t border-gray-100 px-5 py-4 space-y-3">
            <div class="flex items-center justify-between">
                ...
                <span id="mini-cart-subtotal" class="text-[18px] font-black text-heading"><?php echo e(falcon_price_format(0)); ?></span>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <a href="<?php echo e(route('shop.cart')); ?>" class="text-center bg-white text-primary border border-primary px-4 py-2.5 rounded-[3px] text-[13px] font-semibold uppercase tracking-wider hover:bg-gray-50 transition-colors">View Cart</a>
                <a href="<?php echo e(route('shop.checkout')); ?>" class="text-center bg-primary text-white px-4 py-2.5 rounded-[3px] text-[13px] font-semibold uppercase tracking-wider hover:bg-primary-hover transition-colors">Checkout</a>
            </div>
        </div>
    </aside>
</div>
...
window.LazyCart = (function () {
    const ROUTES = {
        add:      <?php echo json_encode(route('shop.cart.add'), 15, 512) ?>,
        fragment: <?php echo json_encode(route('shop.cart.fragment'), 15, 512) ?>,
        removeTpl: <?php echo json_encode(route('shop.cart.remove', ['key' => '__KEY__']), 512) ?>,
    };
...
<?php /**PATH C:\laragon\www\lazy-panda\vendor\tareqcodex\lazy-cms-rebuild\src/../resources/views/themes/falcon-theme/partials/mini-cart.blade.php ENDPATH**/ ?>
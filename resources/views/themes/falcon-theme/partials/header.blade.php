...
<header class="main-header w-full sticky top-0 z-[100]">
            ...
            <nav class="hidden lg:flex items-center gap-8 h-full">
                @php
                    ...
                    try {
                        ...
                        foreach ($__allMM as $__mm) { $__megaMenuLayouts[$__mm['id']] = $__mm; }
                    } catch (\Exception $e) {}
                @endphp
                @foreach($menuItems as $item)
                    @php
                        ...
                        $__hasMegaMenu = !empty($item->mega_menu_id) && isset($__megaMenuLayouts[$item->mega_menu_id]);
                    @endphp
                    <div class="relative group h-full flex items-center" @if($__hasMegaMenu) data-mega-trigger="{{ $item->id }}" @endif>
                        <a href="{{ $item->url }}" class="nav-style {{ $isActive ? 'text-primary' : '' }} hover:text-[{{ $itemHoverColor }}] transition-colors flex items-center gap-1">
                            ...
                            @if($__hasMegaMenu || $item->children->count() > 0)
    ...
    {{-- Mega menu panels: absolute children of sticky header so top:100% = directly below header --}}
    @foreach($menuItems as $item)
        ...
        @if($__hasMM)
            <div class="falcon-mega-panel absolute left-0 right-0 z-50 shadow-xl transition-opacity duration-200"
                ...
                <div class="container-custom py-6">
                    @include('themes.falcon-theme.partials.mega-menu', ['megaMenuData' => $__megaMenuLayouts[$item->mega_menu_id]])
                </div>
            </div>
        @endif
    @endforeach

    @if(!empty($__megaMenuLayouts))
    <script>
    (function(){
        ...
        triggers.forEach(function(trigger) {
            ...
            var panel = document.querySelector('[data-mega-panel="' + itemId + '"]');
            if (!panel) return;

            trigger.addEventListener('mouseenter', function() { showPanel(panel); });
